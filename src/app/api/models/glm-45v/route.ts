...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('GLM-4.5V analysis error:', error);
    return NextResponse.json(
      { 
...
export async function GET() {
  try {
    const modelInfo = await zaiApiService.getModelInfo('glm-45v');
    const isOnline = await zaiApiService.testModelConnection('glm-45v');
    ...
    console.error('GLM-4.5V status check error:', error);
    return NextResponse.json(
      { 
