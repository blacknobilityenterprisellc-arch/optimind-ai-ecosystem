...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('GLM-4.5 Full Stack analysis error:', error);
    return NextResponse.json(
      { 
...
export async function GET() {
  try {
    const modelInfo = await zaiApiService.getModelInfo('glm-45-full-stack');
    const isOnline = await zaiApiService.testModelConnection('glm-45-full-stack');
    ...
    console.error('GLM-4.5 Full Stack status check error:', error);
    return NextResponse.json(
      { 
