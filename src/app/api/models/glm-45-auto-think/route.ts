...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('GLM-4.5 Auto Think analysis error:', error);
    return NextResponse.json(
      { 
...
export async function GET() {
  try {
    const modelInfo = await zaiApiService.getModelInfo('glm-45-auto-think');
    const isOnline = await zaiApiService.testModelConnection('glm-45-auto-think');
    ...
    console.error('GLM-4.5 Auto Think status check error:', error);
    return NextResponse.json(
      { 
